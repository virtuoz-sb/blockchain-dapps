...
import { setIsHarvest, withdrawAll, stakeNft, withdrawNft, STATE, getMyNfts, getStakeNfts, getStakeData } from './olympicStakingSlice'
...
function OlympicStaking() {
    const dispatch = useAppDispatch()
    const stakeData = useAppSelector((state: RootState) => state.olympicStaking.stakeData)
    const isLoadedStake = useAppSelector((state: RootState) => state.olympicStaking.isLoadedStake)
    const displayItems = useAppSelector((state: RootState) => state.olympicStaking.displayItems)
    const isLoaded = useAppSelector((state: RootState) => state.olympicStaking.isLoaded)
    const isLoadedMyStakes = useAppSelector((state: RootState) => state.olympicStaking.isLoadedMyStakes)
    const myStakes = useAppSelector((state: RootState) => state.olympicStaking.myStakes)
    const isHarvest = useAppSelector((state: RootState) => state.olympicStaking.isHarvest)
    ...
    function toggleActiveHarvest(index: number) {
        ...
        if (window.web3 && window.web3.eth && (isConnected === true) && (isCorrectNet === true)) {
            ...
            if (index === 1) {
                ...
                NftStakingV2.methods.getReward()
                ...
                .then((withdrawRes: any) => {
                    ...
                    dispatch(setIsHarvest(0))
                    return dispatch(openNotification(withdrawRes, "Withdraw Olympic"))
                })
                .catch((err: any) => {
                    dispatch(setIsHarvest(0))
                })
    ...
    function stakeIndex(data: any) {
        ...
        if (displayItems[data.index].face_value < 5.0) {
            alertify.dismissAll();
            alertify.error("You can't stake with face value 0")
            // return
        }
        if (window.web3 && window.web3.eth && isConnected && isCorrectNet) {
            ...
            if (data.approve) {
                ERC721_Contract.methods.setApprovalForAll(contractAddress[connectedNetwork].NftStakingV2, true)
                .send({ from: walletAddress, gas: bestGas, gasPrice: bestGasPrice })
                .then((approveRes: any) => {
                    data.type = STATE.SUCC
                    dispatch(stakeNft(data))
                    return dispatch(openNotification(approveRes, "Approve"))
                })
                .catch((err: any) => {
                    data.type = STATE.ERR
                    dispatch(stakeNft(data))
                ...
                let NftStakingV2: any = new window.web3.eth.Contract(contracts.abis.NftStakingV2, contractAddress[connectedNetwork].NftStakingV2)
                console.log("Stake V2", displayItems[data.index].id, displayItems[data.index].stake_powa, BigInt(displayItems[data.index].face_value * 10 ** 18))
                NftStakingV2.methods.stakeNFT(displayItems[data.index].id, displayItems[data.index].stake_powa, BigInt(displayItems[data.index].face_value * 10 ** 18))
                .send({ from: walletAddress, gas: bestGas, gasPrice: bestGasPrice })
                .then((stakeRes: any) => {
                    data.type = STATE.SUCC
                    dispatch(stakeNft(data))
                    return dispatch(openNotification(stakeRes, "Stake Olympic"))
                })
                .catch((err: any) => {
                    data.type = STATE.ERR
                    dispatch(stakeNft(data))
    ...
    function withDraw(index: any) {
        if (window.web3 && window.web3.eth && isConnected && isCorrectNet) {
            ...
            let NftStakingV2: any = new window.web3.eth.Contract(contracts.abis.NftStakingV2, contractAddress[connectedNetwork].NftStakingV2)
            NftStakingV2.methods.withdrawNFT(myStakes[index].id)
            .send({ 
                from: walletAddress, 
                gas: bestGas, 
                gasPrice: bestGasPrice 
            ...
            .then((res: any) => {
                ...
                return dispatch(openNotification(res, "Withdraw Olympic"))
            })
            .catch((err: any) => {
                    ...
                    <div className="bd-shiny active px-0 rounded btn-back-dark h-100">
                        <div className="flex back-blue rounded-top p-4 b-b-blue btn-back-gradient-reverse">
                            ...
                            <div className="flex-col m-auto-v c-white ff-point-cufon">
                                <p className="mb-0 fs-20 bold">Olympic 2020</p>
                                <p className="mb-0"><span className="fc-grey">Stake</span><span className="bold"> Olympic NFT </span><span className="fc-grey">to earn</span><span className="bold"> CIFI</span></p>
                            </div>
                        </div>
                        <div className="flex-col py-4 px-3 white">
                            ...
                            <div className="flex w-100 btn-back-gradient rounded p-1 space-center">
                                <div
                                    className={"text-center w-50 flex rounded p-2 text-lg pointer fc-grey bold harvest-withdraw active-bd-or " + (activeHarvest ? " btn-back-dark active" : "")}
                                                ...
                                                <div className="flex sp-flex-col w-fill">
                                                    ...
                                                    <div className="right w-fit sp-left">
                                                        <p
                                                            ...
                                                            className="bd-shiny rounded-btn-x text-center text-lg pointer bold mb-0 text-center back-dark c-white rounded py-2 px-4"
                                        ...
                                        displayItems.map((item: any, index: any) =>
                                            <div key={index} className="flex-col mb-4">
                                                ...
                                                <div className="flex w-100 btn-back-gradient rounded p-1 space-center pointer mt-4 mb-1">
                                                    <div
                                                        className={"text-center w-50 rounded p-1 text-lg pointer fc-grey bold harvest-withdraw active-bd-or " + (!item.isApproved ? " btn-back-dark active" : "")}
