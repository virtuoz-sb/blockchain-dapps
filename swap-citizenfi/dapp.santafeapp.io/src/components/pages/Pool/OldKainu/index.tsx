...
import { setIsHarvest, withdrawAll, stakeNft, withdrawNft, STATE, getMyNfts, getStakeNfts, getStakeData } from './oldKainuStakingSlice'
...
function OldKainuStaking() {
    const dispatch = useAppDispatch()
    const stakeData = useAppSelector((state: RootState) => state.oldKainuStaking.stakeData)
    const isLoadedStake = useAppSelector((state: RootState) => state.oldKainuStaking.isLoadedStake)
    const displayItems = useAppSelector((state: RootState) => state.oldKainuStaking.displayItems)
    const isLoaded = useAppSelector((state: RootState) => state.oldKainuStaking.isLoaded)
    const isLoadedMyStakes = useAppSelector((state: RootState) => state.oldKainuStaking.isLoadedMyStakes)
    const myStakes = useAppSelector((state: RootState) => state.oldKainuStaking.myStakes)
    const isHarvest = useAppSelector((state: RootState) => state.oldKainuStaking.isHarvest)
    ...
    function toggleActiveHarvest(index: number) {
        ...
        if (window.web3 && window.web3.eth && (isConnected === true) && (isCorrectNet === true)) {
            let OldNftStaking: any = new window.web3.eth.Contract(contracts.abis.OldNftStaking, contractAddress[connectedNetwork].OldNftStaking)
            if (index === 1) {
                dispatch(setIsHarvest(1))
                OldNftStaking.methods.getReward().send({ from: walletAddress, gas: bestGas, gasPrice: bestGasPrice })
                    .then((rewardRes: any) => {
                ...
                OldNftStaking.methods.exit().send({ from: walletAddress, gas: bestGas, gasPrice: bestGasPrice })
    ...
    function stakeIndex(data: any) {
        alertify.dismissAll();
        alertify.error("Please go to main pool page. https://dapp.santafeapp.io/kainu")
        return
        ...
        if (window.web3 && window.web3.eth && isConnected && isCorrectNet) {
            ...
            if (data.approve) {
                ERC721_Contract.methods.setApprovalForAll(contractAddress[connectedNetwork].OldNftStaking, true)
                ...
                let OldNftStaking: any = new window.web3.eth.Contract(contracts.abis.OldNftStaking, contractAddress[connectedNetwork].OldNftStaking)
                OldNftStaking.methods.stakeNFT(displayItems[data.index].id, displayItems[data.index].stake_powa, BigInt(displayItems[data.index].face_value * 10 ** 18)).send({ from: walletAddress, gas: bestGas, gasPrice: bestGasPrice })
                    .then((stakeRes: any) => {
                        data.type = STATE.SUCC
                        dispatch(stakeNft(data))
                    })
    ...
    function withDraw(index: any) {
        if (window.web3 && window.web3.eth && isConnected && isCorrectNet) {
            ...
            let OldNftStaking: any = new window.web3.eth.Contract(contracts.abis.OldNftStaking, contractAddress[connectedNetwork].OldNftStaking)
            OldNftStaking.methods.withdrawNFT(myStakes[index].id).send({ from: walletAddress, gas: bestGas, gasPrice: bestGasPrice })
                .then((res: any) => {
...
export default OldKainuStaking
