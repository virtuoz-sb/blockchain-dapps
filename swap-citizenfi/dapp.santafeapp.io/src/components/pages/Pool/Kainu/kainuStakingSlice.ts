...
export const kainuStakingSlice = createSlice({
    name: 'kainuStaking',
...
export const { setStakeData, setDisplayItems, addDisplayItems, addMyStakes, stakeNft, withdrawNft } = kainuStakingSlice.actions
export const { setMyStakes } = kainuStakingSlice.actions 
export const { withdrawAll, setIsHarvest } = kainuStakingSlice.actions
        ...
        if (validateWalletAddres(walletAddress)) {
            ...
            .then((tokens: any) => {
                if (tokens.length > 0){
                    ...
                    SantaV1.methods.isApprovedForAll(walletAddress, contract.address.NftStaking).call({
                        from: walletAddress
                    })
                    .then((approveRes: any) => {
...
export const getStakeNfts = (walletAddress: any) => {
    return function (dispatch: Dispatch<any>) {
        if (validateWalletAddres(walletAddress)) {
            let NftStaking: any = new polygonWeb3.eth.Contract(contract.abis.NftStaking, contract.address.NftStaking)
            return NftStaking.methods.getNftIds(walletAddress).call({
                from: walletAddress
            })
            .then((tokens: any) => {
...
export const getStakeData = (walletAddress: any) => {
    return function (dispatch: Dispatch<any>) {
        if (validateWalletAddres(walletAddress)) {
            let NftStaking: any = new polygonWeb3.eth.Contract(contract.abis.NftStaking, contract.address.NftStaking)
            // NftStaking.methods._powaBalances("0x4aa2fad040577102be6e879066da2460c4b8e148").call()
            // .then((_powaBalancesRes: any) => {
            //     console.log("my powa value ----------- ", Number(_powaBalancesRes / (10 ** 18)).toFixed(2))
            // })
            return NftStaking.methods._powaBalances(walletAddress).call()
            .then((_powaBalancesRes: any) => {
                    return NftStaking.methods.earned(walletAddress).call()
                    .then((earnedRes: any) => {
                        return NftStaking.methods.getRemainTime().call()
...
export default kainuStakingSlice.reducer