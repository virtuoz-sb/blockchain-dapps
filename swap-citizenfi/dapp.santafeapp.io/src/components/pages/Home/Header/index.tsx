...
import { MENU_LIST, NETWORK_TYPES } from "../../../common/libs/constant";
...
function Header() {
    ...
    const { isConnected, walletAddress, appSetting, initSettting, showConnectWallet, setShowConnectWallet, cifiBalance, networkName, chainIcon, isCorrectNet } = useAppContext()
    ...
    function selectHeaderMenu(menu: MENU_LIST){
        closeMobile() 
    ...
    const nftDropdown: any = [
        {
            label: "Forge",
            link: "/forge",
            hide: NETWORK_TYPES.BSC_MAIN,
            menu: MENU_LIST.FORGE
        },
        // {
        //     label: "LootBox",
        //     link: connectedNetwork === NETWORK_TYPES.BSC_MAIN ? "/" : "/lootbox",
        //     hide: NETWORK_TYPES.MATIC_MAIN
        // },
        // {
        //     label: "Bonds",
        //     link: "/"
            ...
            <nav className="header header-app m-auto-h p-3 flex white ff-point-cufon b-b-gradient pl-4">
                <div className="flex sp-w-100">
                    ...
                    <div className="toggle-mobile mobile-toggle-menu flex sp-flex-col m-auto-v sp-m-2 mx-4 p-2 sp-h-100vh">
                        <Link 
                            to="/" 
                            onClick={() => selectHeaderMenu(MENU_LIST.POOL)} 
                            className={"p-2 text-lg link  m-auto-v sp-my-2 sp-fs-40 sp-my-4 " + (activeHeaderMenu === MENU_LIST.POOL ? "active" : "")}>
                                {i18n.t("words.pools")}</Link>
                        <div className="dropdown nft flex p-2"
                                hidden={connectedNetwork === NETWORK_TYPES.BSC_MAIN} >
                            <div
                                ...
                                aria-expanded="false"
                                data-hover="dropdown">
                                <span className={"m-auto-v fc-grey text-lg sp-fs-40 "+ ( activeHeaderMenu === MENU_LIST.FORGE  ?  "active" : "")}>Oaxaca</span>
                                <span className="m-auto-v">
                                    <i className="icofont-thin-down"></i>
                                </span>
                            </div>
                            <div 
                                className="dropdown-menu dropdown-menu-right btn-back-dark p-3 rounded" 
                                aria-labelledby="dropdownMenuButton" 
                                data-hover="dropdown">
                                {
                                    nftDropdown.map((item: any, index: any) =>
                                        <div 
                                            className="dropdown-item pointer hover-back-dark rounded my-2 py-2 px-3 sp-my-4" 
                                            hidden={connectedNetwork === item.hide}
                                            key={index}>
                                            <Link 
                                                onClick={() => selectHeaderMenu(item.menu)} 
                                                to={item.link} 
                                                className={"hover-white text-lg sp-fs-40  " + (activeHeaderMenu !== item.menu ? "fc-grey" : "white")}>
                                                    {item.label}</Link>
                                        </div>
                                    )
                                }
                        ...
                        <Link hidden={connectedNetwork !== NETWORK_TYPES.MATIC_MAIN} onClick={() => selectHeaderMenu(MENU_LIST.LOOTBOX)} to="/lootbox"     className={"p-2 text-lg link  m-auto-v sp-my-2 sp-fs-40 sp-my-4 " + (activeHeaderMenu === MENU_LIST.LOOTBOX ? "active" : "")}>LootBox</Link>
                        <Link hidden={connectedNetwork === NETWORK_TYPES.MATIC_MAIN || connectedNetwork === NETWORK_TYPES.BSC_MAIN} onClick={() => selectHeaderMenu(MENU_LIST.SWAP)} to="/swap"     className={"p-2 text-lg link  m-auto-v sp-my-2 sp-fs-40 sp-my-4 " + (activeHeaderMenu === MENU_LIST.SWAP ? "active" : "")}>{i18n.t("words.swap")}</Link>
                        
                        <Link 
                            onClick={() => selectHeaderMenu(30)} 
                            hidden={connectedNetwork === NETWORK_TYPES.BSC_MAIN}
                            to={"/market"} 
                            className={"p-2 text-lg link  m-auto-v sp-my-2 sp-fs-40 sp-my-4 " + (activeHeaderMenu === MENU_LIST.MARKET ? "active" : "")}>
                                {i18n.t("words.market_place")}</Link>
                        {
                                ...
                                <>
                                    ...
                                    <div className="dropdown nft flex p-2" >
                                        <div
                                            ...
                                            aria-expanded="false"
                                            data-hover="dropdown">
                                            <span className={"m-auto-v fc-grey text-lg sp-fs-40 "+ ( (activeHeaderMenu >= MENU_LIST.CIFI_PANEL) ?  "active" : "")}>Manage</span>
                                            <span className="m-auto-v">
                                                <i className="icofont-thin-down"></i>
                                            </span>
                                        </div>
                                        <div className="dropdown-menu dropdown-menu-right btn-back-dark p-3 rounded" aria-labelledby="dropdownMenuButton" data-hover="dropdown">
                                            <div className="dropdown-item pointer hover-back-dark rounded my-2 py-2 px-3 sp-my-4">
                                                <Link onClick={() => selectHeaderMenu(MENU_LIST.CIFI_PANEL)} to="/cifipanel" className={"hover-white text-lg sp-fs-40 " + (activeHeaderMenu === MENU_LIST.CIFI_PANEL ? "white" : "fc-grey")}>CIFI Panel</Link>
                                            </div>
                                            <div className="dropdown-item pointer hover-back-dark rounded my-2 py-2 px-3 sp-my-4">
                                                <Link onClick={() => selectHeaderMenu(MENU_LIST.BLIND_PANEL)} to="/blindfipanel" className={"hover-white text-lg sp-fs-40 " + (activeHeaderMenu === MENU_LIST.BLIND_PANEL ? "white" : "fc-grey")}>BlindFi Panel</Link>
                                            </div>
                                            <div className="dropdown-item pointer hover-back-dark rounded my-2 py-2 px-3 sp-my-4">
                                                <Link onClick={() => selectHeaderMenu(MENU_LIST.FAMOSO_BLACK)} to="/famosoblacklist" className={"hover-white text-lg sp-fs-40 " + (activeHeaderMenu === MENU_LIST.FAMOSO_BLACK ? "white" : "fc-grey")}>Famoso Blacklist</Link>
                                            </div>
                                        </div>
                                    </div>
                                </>
                ...
                <div className="flex right sp-bottom-part py-1">
                    <div className="btn-back-dark btn-hover br-6 p-2px flex mx-2 xs-hide" hidden={!isConnected} onClick={() => changeNetwork()}>
                    ...
                    <div className="btn-back-dark btn-hover br-6 p-1 flex wrap-not m-auto-h pointer">
                        {
                            isConnected === true ?
                                isCorrectNet === true ?
                                    <>
                                        <span className="btn-back-dark mx-2 m-auto-v no-wrap">{cifiBalance + " CIFI"}</span>
                    ...
                    <div className="flex mx-2">
                        <div className="dropdown flex" >
                            <div className="dropdown-toggle btn-hover after-none flex pointer m-auto-v" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    ...
                    <div className="flex mx-2 pointer relative">
                        <span className="m-auto btn-hover mx-2 size-24px pointer" id="hompage-setting-global-header-open" onClick={() => openSetting()}>
