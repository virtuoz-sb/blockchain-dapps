...
import InchModal from '@components/ethereum/InchModal';
import useInchDex from '@hooks/moralis/useInchDex';
import { Button, Card, Image, Input, InputNumber, Modal } from 'antd';
import Text from 'antd/lib/typography/Text';
import { ArrowDownOutlined } from '@ant-design/icons';
import { useTokenPrice } from 'react-moralis';
import { tokenValue } from '@common/helpers/formatters';
import { getWrappedNative } from '@common/helpers/networks';
...
function DEX({ chain, customTokens = {} }) {
  ...
  const toTokenAmountUsd = useMemo(() => {
    ...
    return `~$ ${(Moralis?.Units?.FromWei(quote?.toTokenAmount, quote?.toToken?.decimals) * toTokenPriceUsd).toFixed(
      4
    )}`;
          ...
          >
            <div>
              <Input
                ...
                value={
                  quote
                    ? parseFloat(Moralis?.Units?.FromWei(quote?.toTokenAmount, quote?.toToken?.decimals)).toFixed(6)
                    : ''
                }
              />
