...
@Injectable()
export default class OkexProxy {
  public accountType = "spot";

  public quoteCurrency = "USDT";

  private readonly logger = new Logger(OkexProxy.name);

  // help to track ccxt instances accross all users (for debugging purposes)
  private instanceId: string;

  private Exchange = ccxt.okex5;

  private exchangeName = "Okex";
  ...
  private getExchange(cred?: Partial<ExchangeCredentials>) {
    ...
    if ((!cred || cred.useLoadBalancer) && loadBalancer) {
      ...
      const { pathname, hostname } = new URL(baseUrls.rest.replace("{hostname}", defaultHostname));
      exchange.urls.api =
        typeof exchange.urls.api === "string"
          ? loadBalancer
          : {
              ...exchange.urls.api,
              rest: loadBalancer + pathname,
            };
      exchange.headers = exchange.headers || {};
      exchange.headers["x-target-upstream"] = hostname;
    }
    return exchange;
  }
