...
@Injectable()
export default class BinanceProxy {
  ...
  async getKLines(crypto: string, fiat: string, days = 14): Promise<any> {
    const exchange = this.getExchange();
    // last 7 days
    const timestamp = new Date(Date.now() - days * 24 * 60 * 60 * 1000).getTime();
    const markKlines = await exchange.fetchOHLCV(`${crypto}/${fiat}`, "1d", timestamp);
    return markKlines;
  }
