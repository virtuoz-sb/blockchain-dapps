...
@Injectable()
export default class CoinbaseProProxy {
  public accountType = "spot";

  public quoteCurrency = "USDT";

  private readonly logger = new Logger(CoinbaseProProxy.name);

  // help to track ccxt instances accross all users (for debugging purposes)
  private instanceId: string;

  private Exchange = ccxt.coinbasepro;

  private exchangeName = "CoinbasePro";
  ...
  checkCredentialsOnCreation(cred: ExchangeCredentials): Promise<boolean> {
    ...
    return Promise.resolve(account.fetchTotalBalance({ recvWindow: 28000 }).then((res) => true)).catch((err) => {
      this.logger.error(JSON.stringify(err, null, 2), "CoinbaseProProxy");
