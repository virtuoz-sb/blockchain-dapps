...
@Injectable()
export default class BinanceUsProxy {
  public accountType = "spot";

  public quoteCurrency = "USDT";

  private readonly logger = new Logger(BinanceUsProxy.name);

  // help to track ccxt instances accross all users (for debugging purposes)
  private instanceId: string;

  private Exchange = ccxt.binanceus;

  private exchangeName = "binanceus";
  ...
  private getExchange(cred?: Partial<ExchangeCredentials>) {
    ...
    //   const webUrl = new URL(baseUrls.web.replace("{hostname}", defaultHostname));
    ...
    //       : {
    //           ...exchange.urls.api,
    //           web: loadBalancer + webUrl.pathname,
    ...
    return Promise.resolve(
      account
        ...
        .catch((err) => {
          ...
          return { exchange: this.exchangeName, subAccountBalances: [], totalBalances: {}, error: err };
    ...
    return Promise.resolve(account.fetchTotalBalance({ recvWindow: 28000 }).then((res) => true)).catch((err) =>
      Promise.reject(
        new UnprocessableEntityException(
          "Credentials error",
          "Couldn't add your BinanceUS API key. Please double check your public and secret key or retry later."
        )
      )
    );
  }
