  ...
  <div :class="!isDataAvailabe ? 'disabled' : null">
    ...
    <div v-if="isDataAvailabe" class="flex flex-col mb-10 md:mb-0">
      ...
      <div class="flex flex-col">
        <div v-for="(subItems, index) in items.compound.supplied" :key="index" class="pt-8 px-20 mb-6 md:mb-10 last:mb-0 md:last:mb-10">
          <div v-for="(item, j) in subItems" :key="j" class="grid grid-cols-4 md:grid-cols-5 col-gap-10 items-center">
            ...
            <div class="truncate text-iceberg text-xs md:text-md">{{ item.balance | fixed(6, 0) }}</div>

            <div class="truncate text-iceberg text-xs md:text-md">${{ item.usdValue | fixed(2, 0) }}</div>

            <div class="truncate text-iceberg leading-xs text-xs md:text-md">{{ (item.apr * 100) | fixed(2, 0) }}%</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <template v-if="isDataAvailabe">
      <div v-if="items.compound.borrowed && items.compound.borrowed.length" class="flex flex-col">
        <!-- TITLE FOR MOBILE VIEW -->
        <div v-if="$breakpoint.smAndDown" class="flex w-full pt-20 px-20 mb-15">
        ...
        <!-- LIST ITEMS -->
        <div class="flex flex-col">
          <div v-for="(subItems, index) in items.compound.borrowed" :key="index" class="pt-8 px-20 mb-6 md:mb-10 last:mb-0 md:last:mb-10">
            <div v-for="(item, j) in subItems" :key="j" class="grid grid-cols-4 md:grid-cols-5 col-gap-10 items-center">
              <div v-if="!$breakpoint.smAndDown" />
...
@Component({ name: "CompoundDetails" })
export default class CompoundDetails extends Vue {
  /* PROPS */
  @Prop({ required: true }) items: ProjectsData;

  /* COMPUTED */
  get isDataAvailabe() {
    return Boolean(this.items && this.items.compound && this.items.compound.borrowed.length);
  }

  get suppliedData() {
    if (!this.$breakpoint.smAndDown) {
      return ["Supplied", "Token", "Balance", "Value", "APY"];
    } else {
      return ["Token", "Balance", "Value", "APY"];
    }
  }

  get borrowedData() {
    if (!this.$breakpoint.smAndDown) {
      return ["Borrowed", "Token", "Balance", "Value", "APY"];
    } else {
      return ["Token", "Balance", "Value", "APY"];
    }
  }
