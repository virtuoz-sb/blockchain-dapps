<template>
  <div :class="!isDataAvailabe ? 'disabled' : null">
    ...
    <div v-if="isDataAvailabe" class="flex flex-col mb-10 md:mb-0">
      ...
      <!-- ITEMS LIST -->
      <div v-if="items.uniswapV2 && items.uniswapV2.length">
        <div
          v-for="(item, index) in items.uniswapV2"
          :key="index"
...
@Component({ name: "UniswapDetails" })
export default class UniswapDetails extends Vue {
  /* PROPS */
  @Prop({ required: true }) items: ProjectsData;

  /* COMPUTED */
  get isDataAvailabe() {
    return Boolean(this.items && this.items.uniswapV2 && this.items.uniswapV2.length);
  }
