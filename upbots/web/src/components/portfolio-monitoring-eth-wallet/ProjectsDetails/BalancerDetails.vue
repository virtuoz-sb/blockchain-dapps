<template>
  <div :class="!isDataAvailabe ? 'disabled' : null">
    ...
    <div v-if="isDataAvailabe" class="flex flex-col mb-10 md:mb-0">
      ...
      <!-- ITEMS LIST -->
      <div v-if="items.balancer && items.balancer.length">
        <div
          v-for="(item, index) in items.balancer"
          :key="index"
        ...
        >
          ...
          <div class="flex items-center">
            ...
            <div class="truncate text-iceberg leading-xs text-xs md:text-md ml-10">
              <span v-for="(token, index) in item.tokens" :key="index">
                {{ token.tickerSymbol }}
              </span>
            </div>
          </div>

          <span class="truncate text-iceberg leading-xs text-xs md:text-md">
            <span v-for="(token, index) in item.tokens" :key="index">
              {{ token.balance | fixed(3, 0) }} {{ token.tickerSymbol }}
              <span v-if="index < item.tokens.length - 1">+</span>
            </span>
          </span>
...
@Component({ name: "BalancerDetails" })
export default class BalancerDetails extends Vue {
  /* PROPS */
  @Prop({ required: true }) items: ProjectsData;

  /* COMPUTED */
  get isDataAvailabe() {
    return Boolean(this.items && this.items.balancer && this.items.balancer.length);
  }
