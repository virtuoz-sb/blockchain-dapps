<template>
  <div :class="!isDataAvailabe ? 'disabled' : null">
    ...
    <div v-if="isDataAvailabe" class="flex flex-col mb-10 md:mb-0">
      ...
      <!-- ITEMS LIST -->
      <div v-if="items.pancakeswap && items.pancakeswap.length">
        <div
          v-for="(item, index) in items.pancakeswap"
          :key="index"
        ...
        >
          ...
          <div class="truncate text-iceberg leading-xs text-xs md:text-md">
            {{ item.tokens[0].balance | fixed(3, 0) }} {{ item.tokens[0].tickerSymbol }} + {{ item.tokens[1].balance.toFixed(2) }}
...
@Component({ name: "PancakeswapDetails" })
export default class PancakeswapDetails extends Vue {
  /* PROPS */
  @Prop({ required: true }) items: ProjectsData;

  /* COMPUTED */
  get isDataAvailabe() {
    return Boolean(this.items && this.items.pancakeswap && this.items.pancakeswap.length);
  }
