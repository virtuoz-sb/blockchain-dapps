  ...
  <tbody>
    <tr>
      <td class="relative table_column_1 pt-10 pb-10 pl-0">
        <div v-if="data.details" :class="['icon-expandable', this.isCollapsed ? 'collapsed' : 'expanded']" @click="toggleCollapsed()" />
        <div class="flex justify-end">
          <img
            v-if="data.type === 'DEPOSIT'"
            src="/img/ubxt-balance/deposit.png"
            :alt="data.type"
            class="w-24 h-24 text-iceberg text-base bg-opacity-0"
          />
          <img v-else src="/img/ubxt-balance/payment2.png" :alt="data.type" class="w-24 h-24 text-iceberg text-sm" />
        </div>
      </td>
      <td class="table_column_1 pt-10 pb-10 pl-0">
        <div class="flex">
          <span v-if="data.type" class="text-iceberg px-10 text-base">{{ getType() }}</span>
      ...
      <td class="table_column_2 pt-10 pb-10">
        <div class="flex items-center">
          <span v-if="data.status" class="px-10 text-base text-iceberg">
            {{ getStatus() }}
          </span>
          <span v-else class="flex bg-grey-100 w-full h-px mr-20" />
    ...
    <template v-if="data.details && !isCollapsed">
      <UBXTTransactionItemSubItem v-for="(item, index) in data.details" :key="index" :data="item" />
    </template>
  </tbody>
</template>
...
import { TransactionTypes, TransferType, TransactionStatuses } from "@/store/perfees/types";
import UBXTTransactionItemSubItem from "@/components/ubxt-wallet/ubxt-transaction-table/UBXTTransactionItemSubItem.vue";

@Component({ name: "UBXTTransactionItem", components: { UBXTTransactionItemSubItem } })
export default class UBXTTransactionItem extends Vue {
  ...
  getStatus() {
    let status = this.getCapitalizeText(this.data.status);
    if (
      (this.data.type === TransactionTypes.WITHDRAW || this.data.type === TransactionTypes.PERFORMANCE_FEE) &&
      this.data.subType === TransferType.WITHDRAW &&
      (this.data.status === TransactionStatuses.PENDING || this.data.status === TransactionStatuses.CONFIRMED)
    ) {
      status = "Sent";
    }
    return status;
  ...
  getType() {
    ...
    if (this.data.type === TransactionTypes.PERFORMANCE_FEE && this.data.subType) {
      type = `Perf-fee (${this.getCapitalizeText(this.data.subType)})`;
    }
    return type;
  }
...
.icon-expandable {
  ...
  &.expanded {
    background-image: url("~@/assets/icons/minus-square.svg");
  }
}
.error-circle {
  height: 16px;
