<template>
  <div class="text-grey-cl-920 border-t py-15 last:mb-0">
    <!-- TYPE -->
    <div class="grid grid-cols-2 col-gap-20 items-center mb-20">
      <p class="leading-xs">Sub Type:</p>

      <div class="flex items-center">
        <div class="flex">
          <span v-if="data.type" class="leading-xs">{{ getCapitalizeText(data.subType) }}</span>
    ...
    <div class="grid grid-cols-2 col-gap-20 items-center mb-20 tooltip-box">
      <p class="leading-xs">Status:</p>

      <div class="flex items-center">
        <span v-if="data.status" class="leading-xs">{{ getCapitalizeText(data.status) }}</span>
    ...
    <!-- AMOUNT -->
    <div class="grid grid-cols-2 col-gap-20 items-center mb-20">
      <p class="leading-xs">Amount:</p>

      <div class="flex">
        <span v-if="data.amount" class="leading-xs">
    ...
    <!-- CONFIRM% -->
    <div class="grid grid-cols-2 col-gap-20 items-center mb-20">
      <p class="leading-xs">Confirm%:</p>

      <div class="flex">
        <span v-if="data.confirmPercent" class="leading-xs">{{ confirmPercent }}</span>
        <span v-else class="flex bg-grey-100 w-full h-px mr-20" />
      </div>
    </div>
    ...
    <!-- DATE -->
    <div class="grid grid-cols-2 col-gap-20 items-center mb-20">
      <p class="leading-xs">Date:</p>

      <div class="flex">
        <span v-if="data.createdAt" class="leading-xs"> {{ getDate(data.createdAt) }}</span>
        <span v-else class="flex bg-grey-100 w-full h-px mr-20" />
      </div>
    </div>
    ...
    <div class="grid grid-cols-2 col-gap-20 items-center">
      <p class="leading-xs">Transaction Hash:</p>

      <div class="flex">
        <span v-if="data.hash" class="text-sm">
...
@Component({ name: "UBXTTransactionItemMobile" })
