...
class AuthService {
  ...
  async deactive2FASecret (payload: ITotpTokenRequest) {
    const res = await http.post<ILoginResponse>('/auth/totp-deactivate', payload)
    return res.data
  }
  ...
  async changePwd (payload: ChangePwdRequest) {
    const res = await http.post<boolean>('/auth/change-password', payload)
    return res.data
  }
}
