...
import { BOT_LOAD_ALL, BOT_UPDATE, BOT_DELETE, BOT_LOAD_MY, BOT_ADD, BOT_START, BOT_STOP, BOT_LOAD_STATUSES, BOT_ARCHIVE, BOT_RESPONSE_WAITING, BOT_SEARCH } from '../action-types';
...
export const botReducer: Reducer<BotState, BotAction> = handleActions(
  {
    ...
    [BOT_SEARCH]: (state: BotState, { payload: { bots, total }}: BotAction) => ({
      ...state,
      bots,
      total: total
    }),
    ...
    [BOT_RESPONSE_WAITING]: (state: BotState, { payload: { botId }}: BotAction) => {
      const bots = state.bots.map(item => {
        if (item._id === botId) {
          const temp = {
            ...item.state,
    ...
    [BOT_LOAD_STATUSES]: (state: BotState, { payload: { statuses }}: BotAction) => {
      const bots = state.bots.map(item => {
        const temp = statuses.find(status => status._id === item._id && item.state.status !== ERunningStatus.WAITING);
        if (temp) {
          return {
            ...item,
            state: temp.state
